...
<%
	...
	try {
		...
		if (!(sql.executeUpdate("INSERT INTO [jspforum].[dbo].[post] VALUES('" + session.getAttribute("userid") + "', '" + sqlDate + 
			"', '', '" + request.getParameter("Detail") +"', '" +  request.getParameter("postid") + "')") == 1)) {
			out.print("评论失败！");
		}
		else {
			out.print("评论成功！");
...
%>		<form action="onePost.jsp" method=post name=form>
			<br><input type="submit" name="submit" value="回到原帖">
